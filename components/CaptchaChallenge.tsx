...
const CaptchaChallenge: React.FC<CaptchaChallengeProps> = ({ onVerify, onSuccess, onStart, onMilestone, onGameOver, onScoreUpdate, onSessionRewardUpdate, isMining, isSoundEnabled }) => {
    ...
    useEffect(() => {
        ...
        bgMusicAudio.current = new Audio('/sounds/birds-chirping.wav');
        ...
        charImg.src = '/pepepe.png';
        charImg.onload = () => { characterSpriteRef.current = charImg; };

        // Load PEPE background
        const pepeBgImg = new Image();
        pepeBgImg.src = '/pepe_background.png';
        pepeBgImg.onload = () => { flagBgRef.current = pepeBgImg; };
    ...
    useEffect(() => {
        if (!bgMusicAudio.current) return;

        if (isSoundEnabled && gameState === 'PLAYING') {
            bgMusicAudio.current.volume = volume;
            bgMusicAudio.current.play().catch(e => console.error("BG Music play failed:", e));
        } else {
            bgMusicAudio.current.pause();
        }
    }, [isSoundEnabled, gameState, volume]);
    ...
    const jump = useCallback(() => {
        ...
        if (p.grounded) {
            ...
            p.grounded = false;
            playSound(jumpAudio.current); // Play jump sound
        }
    }, [gameState, initGame]);
        ...
        const loop = (timestamp: number) => {
            ...
            const drawBackground = () => {
                ...
                if (flagBgRef.current) {
                    ctx.drawImage(flagBgRef.current, 0, 0, width, height);
                } else {
                    // Fallback: PEPE green gradient
                    const gradient = ctx.createLinearGradient(0, 0, 0, height);
                    gradient.addColorStop(0, '#87ceeb'); // Light blue sky
                    gradient.addColorStop(0.4, '#a8d8a8'); // Light green
                    gradient.addColorStop(1, '#6fa838'); // PEPE green
            ...
            // Ground - PEPE swampy green surface
            ctx.fillStyle = '#2d5016'; // Dark swampy green
            ...
            obstaclesRef.current.forEach(obs => {
                if (obs.type === 'duststorm') {
                    ...
                    // Draw muddy swamp obstacles
                    const rockGradient = ctx.createLinearGradient(0, -obs.height, 0, 0);
                    rockGradient.addColorStop(0, '#5c4033'); // Dark brown
                    rockGradient.addColorStop(1, '#3d2817'); // Muddy brown
    ...
    return (
        <div className="relative w-full h-full">
            <canvas
                ...
                className={`w-full h-full bg-[#6fa838] transition-all duration-300 cursor-pointer
