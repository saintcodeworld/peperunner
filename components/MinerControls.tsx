...
const MinerControls: React.FC<MinerControlsProps> = ({
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <div className="bg-emerald-950/40 backdrop-blur-xl border border-emerald-500/20 rounded-2xl p-6 space-y-4 shadow-2xl relative overflow-hidden transition-all duration-500">
        {/* Subtle Glow Overlay */}
        ...
        <div className="flex items-start justify-between cursor-pointer group" onClick={() => setShowWithdrawSettings(!showWithdrawSettings)}>
          <div>
            <div className="flex items-center gap-2 mb-2">
              ...
              <h3 className="text-emerald-400 text-xs font-bold uppercase">Earned Balance 🐸</h3>
            </div>
            <div className="text-2xl font-black text-emerald-300 mono tracking-tight">
              {currentBalance.toFixed(6)} <span className="text-sm font-bold text-emerald-400">SOL</span>
            </div>
          </div>
          <div className="text-right">
            <div className="text-[10px] font-bold text-emerald-400 uppercase tracking-widest mb-1">Minimum</div>
            <div className="text-xs font-mono text-emerald-300 bg-emerald-900/50 px-2 py-1 rounded border border-emerald-700/50">0.03 SOL</div>
          </div>
        </div>
        ...
        <div className={`transition-all duration-500 ease-in-out overflow-hidden ${showWithdrawSettings ? 'max-h-96 opacity-100 mt-4' : 'max-h-0 opacity-0'}`}>
          <div className="bg-emerald-950/50 rounded-xl p-4 space-y-4 border border-emerald-800/50">
            <div>
              <label className="text-[10px] uppercase text-emerald-500 font-bold block mb-1.5 tracking-wider">Destination Wallet</label>
              <div className="relative">
                <input
                  type="text"
                  readOnly
                  value={config.payoutAddress || "Loading..."}
                  className="w-full bg-black/40 border border-emerald-800 rounded-lg px-3 py-2 text-xs text-emerald-300 font-mono focus:border-emerald-600 outline-none"
                />
              </div>
            </div>
            <div>
              <label className="text-[10px] uppercase text-emerald-500 font-bold block mb-1.5 tracking-wider">Withdrawal Amount</label>
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="w-full bg-black/40 border border-emerald-800 rounded-lg px-3 py-2 text-xs text-emerald-300 font-mono focus:border-emerald-600 outline-none"
                />
                <span className="text-[10px] text-emerald-400 font-bold px-2 py-1 bg-emerald-900/50 rounded border border-emerald-700/50">MAX</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={`neo-btn w-full ${canWithdraw ? 'bg-gradient-to-r from-emerald-600 to-green-500 border-emerald-500 shadow-[0_4px_15px_rgba(34,197,94,0.3)] hover:shadow-[0_6px_20px_rgba(34,197,94,0.5)]' : 'bg-emerald-950 border-emerald-800 text-emerald-600'} ${isWithdrawing ? 'opacity-70 cursor-wait' : ''}`}
          >
            {isWithdrawing ? (
              <>
        ...
        {withdrawError && (
          <div className="bg-red-900/10 border border-red-500/30 rounded-lg p-2 mt-2">
        ...
        {withdrawSuccess && (
          <div className="bg-emerald-500/10 border border-emerald-500/30 rounded-lg p-2 mt-2">
            <p className="text-xs text-emerald-400 font-medium text-center">{withdrawSuccess}</p>
          </div>
        )}
